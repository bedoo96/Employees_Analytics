...
if st.session_state.df is not None:
    ...
    st.markdown("""
    Export comprehensive attendance reports for HR follow-up and management review.
    Reports include automated insights, action items, and recommendations.
    """)
    ...
    with col1:
        if st.button("📊 Generate Excel Report", type="secondary", use_container_width=True):
            with st.spinner("Generating Excel report with insights..."):
                try:
                    report_gen = ReportGenerator(analyzer)
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    excel_path = f"/tmp/attendance_report_{timestamp}.xlsx"
                    report_gen.generate_excel_report(excel_path)
                    ...
                    **Excel Report includes:**
                    ...
                    - Detailed Employee Data with HR notes
    ...
    with col2:
        if st.button("📑 Generate PDF Report", type="secondary", use_container_width=True):
            with st.spinner("Generating PDF report for HR..."):
                try:
                    report_gen = ReportGenerator(analyzer)
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    pdf_path = f"/tmp/attendance_report_{timestamp}.pdf"
                    report_gen.generate_pdf_report(pdf_path)
                    
                    with open(pdf_path, 'rb') as f:
                        st.download_button(
                            label="⬇️ Download PDF Report",
                            data=f.read(),
                            file_name=f"attendance_report_{timestamp}.pdf",
    ...
    with col3:
        if st.button("📦 Generate Both Reports", type="primary", use_container_width=True):
            with st.spinner("Generating both Excel and PDF reports..."):
                try:
                    report_gen = ReportGenerator(analyzer)
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    ...
                    report_gen.generate_both_reports(excel_path, pdf_path)
